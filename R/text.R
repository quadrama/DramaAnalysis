...
#' stylo_table <- make_stylo_table(tl, names=TRUE, by.figure=TRUE)
#' stylo(gui=F, frequencies = stylo_table, network=T, write.png.file=T, analysis.type="BCT")
make_stylo_table <- function(t, accepted.pos=postags$de$words, names=FALSE, column="Token.surface", by.figure=FALSE) {
  ft <- t
  if (length(accepted.pos) > 0)
