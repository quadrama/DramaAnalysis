...
b <- characterStatistics(drama, normalize = FALSE) %>%
  characterNames(drama) %>%
  barplot(#names.arg=dramaNames(drama, formatString = "%T"),
          column="tokens",
          main = "Absolute amount of words spoken by each character (Tokens).")
...
b <- characterStatistics(drama, normalize = FALSE) %>%
  ...
  barplot(#names.arg=dramaNames(drama, formatString = "%T"),
          column="utterances",
          main= "Absolute amount of utterances of each character.")
```
...
b <- characterStatistics(drama, normalize = FALSE) %>%
  characterNames(drama) %>%
  barplot(#names.arg=dramaNames(drama, formatString = "%T"),
          column="utteranceLengthMean",
          main="Mean of the length of the utterances of each character.")
```
...
b <- characterStatistics(drama, normalize = FALSE) %>%
  ...
  barplot(#names.arg=dramaNames(drama, formatString = "%T"),
          column="types",
          main="Absolute amount of different Words spoken by each character (Types).")
```
...
### Heatmap
```{r, echo=FALSE}
c <- configuration(drama, onlyPresence=TRUE, segment="Scene") %>%
  filterCharacters(drama, n=7) %>%
  characterNames(drama)
...
### By Act
```{r, echo=FALSE, results="asis"}
table <- characterNames(configuration(drama, segment = "Act"), drama)
...
### By Scene 
```{r, echo=FALSE, results="asis"}
table <- characterNames(configuration(drama, segment = "Scene"), drama)
...
Graph density: `r graph.density(g)`

Edge connectivity: `r edge_connectivity(g)`
...
This report has been generated using the R package `DramaAnalysis` in version `r pver`. The package is being developed [here](http://github.com/quadrama/DramaAnalysis), in the context of the project [QuaDramA](https://quadrama.github.io). Please report errors [here](https://github.com/quadrama/DramaAnalysis/issues), or contact [me](mailto:nils.reiter@ims.uni-stuttgart.de).
