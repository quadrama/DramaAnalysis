data("rksp.0.text")
data("vndf.0.text")

require(data.table)

toler <- 1e-4

testtext <- data.table(drama=c("a","a","a"),
                       Speaker.figure_id=c(1, 1, 2),
                       Token.surface=c("b","b","a"))

ft <- frequencytable(testtext, byFigure = TRUE, acceptedPOS=c(),sortResult=FALSE)
expect_equal(ft[1,1], 0, tolerance=toler)
expect_equal(ft[1,2],0.666667, tolerance=toler)
expect_equal(ft[2,1],0.3333, tolerance=toler)
expect_equal(ft[2,2],0, tolerance=toler)

text <- rbind(vndf.0.text,rksp.0.text)

ft <- frequencytable(text,normalize=FALSE,byFigure = FALSE)

