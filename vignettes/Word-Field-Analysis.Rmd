...
Since version 2.0 of this package, we can also count word fields by segment, i.e., by act or scene in a play. To do that, we add the parameter `segment="Act"` when we call the function `dictionaryStatistics()`. We can now easily plot the theme progress over the course of the play:
...
matplot(as.matrix(dsl[dsl$character=="marinelli",]), type="l", col="black")
legend(x="topleft",legend=colnames(as.matrix(dsl)),lty=1:5)
...
```{r}
mat <- apply(as.matrix(dsl[dsl$character=="marinelli",]),2,cumsum)
matplot(mat,type="l",col="black")

# Add act lines
al <- rle(dsl[dsl$character=="marinelli",]$Number.Act)
