...
par(mar=c(2,2,2,10))
barplot(cList$matrix, 
        legend.text = cList$figure, # legend text
        args.legend = list(cex=0.5, # legend font size
                           x=7.5, # legend x position
                           y=max(colSums(cList$matrix)) # legend y pos
                        ), 
        col=qd.colors)
```
...
```{r}
cList$matrix <- scale(cList$matrix, center=FALSE, scale=colSums(cList$matrix))
cList$matrix
par(mar=c(2,2,2,10))
barplot(cList$matrix, 
        legend.text=cList$figure, # set legend text
        args.legend = list(cex=0.5, # legend font size
                           x=7.5, # legend x position
                           y=max(colSums(cList$matrix)) # legend y pos
                        ),
...
```{r}
c <- configuration(rksp.0, onlyPresence = TRUE, by="Scene")
...
copresence <- cList$matrix %*% t(cList$matrix)
copresence

# add figure names
rownames(copresence) <- cList$figure
colnames(copresence) <- cList$figure
```
...
# add the x axis
axis(1, at = seq(0,1,length.out = length(cList$figure)), labels = cList$figure, las=3)
# add the y axis
axis(2, at = seq(0,1,length.out = length(cList$figure)), labels = cList$figure, las=1)
```
